...
- **Mechanically enforced** — in Claude Code and Codex these boundaries are live policy, not
  just prose: the checked-in `.claude/settings.json` / `.codex/hooks.json` (and the user-level
  configs setup.sh installs) run `scripts/hooks/pretooluse-guard.cjs` before every shell/edit
  tool call (git mutations, secret reads, and protected-file edits deny; `push` prompts),
  husky refuses agent-shell commits, and the CLI refuses non-interactive `push` without
  `--yes`. Policy changes go through `scripts/hooks/guard-core.cjs` + `test/hooks.test.cjs`
  together. Other tools (Cursor) rely on this prose alone.
